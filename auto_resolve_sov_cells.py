...
# -----------------------------
# TOTAL SF inference from PDFs
# -----------------------------

_SF_RX = re.compile(r'\b(\d[\d,]{2,8})\s*(?:SF|S\.F\.|SQ\.?\s*FT|SQFT|GSF|NSF)\b', re.IGNORECASE)

# Positive/negative context scoring to pick the "right" SF when multiple are present
_SF_POS = [
    (re.compile(r'\b(PROPOSED|TENANT|TOTAL)\b', re.I), 8),
    (re.compile(r'\b(LEASE\s*AREA)\b', re.I), 6),
    (re.compile(r'\b(FLOOR\s*AREA|AREA)\b', re.I), 3),
]
_SF_NEG = [
    (re.compile(r'\b(L\.L\.|LL)\b', re.I), -20),
    (re.compile(r'\b(LANDLORD)\b', re.I), -20),
    (re.compile(r'\b(ALLOWABLE)\b', re.I), -8),
    (re.compile(r'\b(OCCUPAN|EGRESS)\b', re.I), -6),
]

def pick_best_sf_from_text(flat: str):
    """Return (best_sf_int, debug_dict) from a flattened text string."""
    if not flat or not isinstance(flat, str):
        return None, {"cands": []}

    cands = []
    for m in _SF_RX.finditer(flat):
        raw = m.group(1)
        try:
            sf = int(raw.replace(",", ""))
        except Exception:
        ...
        score += max(0, 5 - int(m.start() / max(1, len(flat)) * 5))

        cands.append({"sf": sf, "score": score, "match": m.group(0), "ctx": ctx})

    if not cands:
        return None, {"cands": []}

    cands.sort(key=lambda x: (x["score"], x["sf"]), reverse=True)
    return cands[0]["sf"], {"cands": cands[:10], "picked": cands[0]}

def infer_total_sf_from_plan_sources(plan: dict, max_candidates: int = 10):
    """
    Walk likely project PDFs (excluding Sub Bids) and infer TOTAL_SF.
    Uses extract_lines() if present; OCR retry via ocr_pdf() if needed.
    Returns (sf_int_or_None, debug_dict).
    """
    try:
        writes = plan.get("writes", [])
    except Exception:
        return None, {"why": "no plan/writes"}

    # derive base folder from any pdf sources in the plan
    pdfs = []
    for w in writes:
        ...
        if isinstance(sp, str) and sp.lower().endswith(".pdf"):
            pdfs.append(Path(sp))
    if not pdfs:
        return None, {"why": "no pdf sources in plan"}
    ...
    # prioritize permit sets / drawings / bid package / specs / cover sheets
    kw = re.compile(r'(permit|set|plan|plans|drawing|drawings|cover|title|spec|specs|bid\s*package|invitation|itb)', re.I)
    ...
    for fp in base.rglob("*.pdf"):
        ...
        if "sub bids" in s:
            continue
        # prefer keyworded PDFs first
        pri = 0
        if kw.search(fp.name) or kw.search(str(fp.parent)):
            pri += 10
        if "permit" in fp.name.lower():
            pri += 5
        candidates.append((pri, fp))

    candidates.sort(key=lambda x: x[0], reverse=True)
    candidates = [fp for _, fp in candidates[:max_candidates]]

    dbg_hits = []
    best = None
    best_score = -10**9

    # helpers expected to exist in your file after prior wiring:
    #  - extract_lines(path, maxpages=...)
    #  - ocr_pdf(path, mode=...)
    for fp in candidates:
        # try text-layer first
        try:
            lines = extract_lines(fp, maxpages=12)
        except Exception:
            lines = []

        flat = " ".join(" ".join(lines).split()) if lines else ""
        sf, info = pick_best_sf_from_text(flat) if flat else (None, {"cands": []})

        # if not found, OCR + retry
        if sf is None:
            try:
                o = ocr_pdf(fp, mode="skip_text")
                lines2 = extract_lines(o, maxpages=16)
                flat2 = " ".join(" ".join(lines2).split()) if lines2 else ""
                sf2, info2 = pick_best_sf_from_text(flat2) if flat2 else (None, {"cands": []})
                sf, info = sf2, info2
            except Exception:
                pass

        if sf is not None:
            # use picked score as ranking signal
            picked = (info.get("picked") or {})
            score = int(picked.get("score", 0))
            dbg_hits.append({"pdf": str(fp), "sf": sf, "score": score, "picked": picked})
            if score > best_score:
                best_score = score
                best = sf

    return best, {"hits": dbg_hits[:10], "best_score": best_score, "scanned": [str(x) for x in candidates]}
...
def resolve_header_blocks(ws, plan, source, meta, args):
    ...
    try:
        ...
        # --- PROJECT LOCATION OVERRIDE (schema/project_locations.json, in-scope) ---
        try:
            from pathlib import Path as _Path
            import json as _json
            _loc_path = _Path.home()/"TCL_BRAIN"/"schema"/"project_locations.json"
            _loc_map = _json.loads(_loc_path.read_text(encoding="utf-8")) if _loc_path.exists() else {}
            _keys = []
            try:
                if isinstance(vals, dict):
                    _p = vals.get("PROJECT")
                    _j = vals.get("JOB NO.")
                    if isinstance(_p, str) and _p.strip(): _keys.append(_p.strip())
                    if isinstance(_j, str) and _j.strip(): _keys.append(_j.strip())
            except Exception:
                pass
            try:
                if isinstance(default_meta, dict):
                    _pm = default_meta.get("project")
                    if isinstance(_pm, str) and _pm.strip(): _keys.append(_pm.strip())
            except Exception:
                pass
            _ov = None
            for _k in _keys:
                ...
                if isinstance(_v, str) and _v.strip():
                    _ov = _v.strip()
                    break
            # apply to vals if missing
            if isinstance(vals, dict) and (not vals.get("LOCATION")) and _ov:
                # guard against office address denylist if present
                try:
                    if ' _DENY_ADDR' and callable(globals().get('_DENY_ADDR', None)):
                        pass
                except Exception:
                    pass
                vals["LOCATION"] = _ov
            # ensure plan has HEADER:LOCATION write at O4 (update if exists, else append)
            _loc_final = vals.get("LOCATION") if isinstance(vals, dict) else None
            if isinstance(_loc_final, str) and _loc_final.strip():
                _found = False
                for _w in plan.get("writes", []):
                    if not isinstance(_w, dict):
                        continue
                    _m = _w.get("meta") or {}
                    if _m.get("trade") == "HEADER" and _m.get("line_id") == "HEADER:LOCATION":
                        # normalize to O4
                        _w["cell"] = "O4"
                        if isinstance(_w.get("write"), dict):
                            _w["write"]["value"] = _loc_final
                        else:
                            _w["value"] = _loc_final
                        _found = True
                        break
                if not _found:
                    plan.setdefault("writes", []).append({
                        "sheet": ws.title,
                        "cell": "O4",
                        "write": {"value": _loc_final},
                        "source": default_source,
                        "meta": {**(default_meta or {}), "trade": "HEADER", "bucket_code": "HEADER", "line_id": "HEADER:LOCATION"},
                    })
        except Exception:
            pass
        # --- END PROJECT LOCATION OVERRIDE (schema/project_locations.json, in-scope) ---

        pass

    # --- PROJECT_LOC_OVERRIDE_ENDV1: project_locations.json override (project address, never TCL office) ---
    try:
        import json as _json
        from pathlib import Path as _Path
        _loc_path = _Path.home()/"TCL_BRAIN"/"schema"/"project_locations.json"
        ...
        for _k in _keys:
            ...
            if isinstance(_v, str) and _v.strip():
                _ov = _v.strip()
                break
        print("LOC_OV_END_KEYS:", _keys)
        print("LOC_OV_END_VAL_RAW:", repr(_ov))
        # sanitize to address-like + denylist
        _ov2 = _pick_best_address(_ov) if isinstance(_ov, str) else None
        print("LOC_OV_END_VAL_PICKED:", repr(_ov2))
        if isinstance(vals, dict) and (not vals.get("LOCATION")) and _ov2:
            vals["LOCATION"] = _ov2
            print("LOC_OV_END_APPLIED: True")
        else:
            print("LOC_OV_END_APPLIED: False")
        # ensure a HEADER:LOCATION write exists if we have LOCATION
        _loc_final = vals.get("LOCATION") if isinstance(vals, dict) else None
        if isinstance(_loc_final, str) and _loc_final.strip():
            _found = False
            for _w in plan.get("writes", []):
            ...
            if not _found:
                plan.setdefault("writes", []).append({
                    ...
                    "write": {"value": _loc_final},
                    "source": source,
                    "meta": {**(meta or {}), "trade": "HEADER", "bucket_code": "HEADER", "line_id": "HEADER:LOCATION"},
                })
            print("LOC_OV_END_WRITE_APPENDED: True")
    except Exception as _e:
        print("LOC_OV_END_ERR:", _e)
    # --- END PROJECT_LOC_OVERRIDE_ENDV1 ---


def resolve_sf_blocks(ws, plan, source, meta, args):
    # --- AUTO INFER TOTAL_SF if not provided ---
    total_sf = getattr(args, 'total_sf', None)
    if total_sf is None:
        try:
            sf_guess, sf_dbg = infer_total_sf_from_plan_sources(plan)
            if sf_guess is not None:
                total_sf = sf_guess
                print('INFERRED_TOTAL_SF:', total_sf)
        except Exception as _e:
            print('INFER_TOTAL_SF_ERR:', _e)
    # --- END AUTO INFER TOTAL_SF ---
    ...
    if total_sf is not None:
        ...
        if hit:
            r,c = hit
            rr,cc = cell_right_of(ws, r, c)
            add_write(plan, f"{col_letter(cc)}{rr}", total_sf, "ESTIMATE (INPUT)", source, meta)
...
def main():
    ...
    # --- HARDENING: restamp template_sha256 from lock at write-time ---
    try:
        lock_path = plan.get("lock")
        if lock_path:
